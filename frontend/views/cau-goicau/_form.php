...
<div class="cau-goicau-form">
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <?= $form->field($model, 'trennhip')->textInput(['maxlength' => true]) ?>
        </div>
        <div class="col-lg-3">
            <?= $form->field($model, 'trenmotru')->textInput(['maxlength' => true]) ?>
        </div>
        <div class="col-lg-3">
            <?=
            $form->field($model, 'danglienket')->dropDownList(
                    ArrayHelper::map(CauThongtinchung::find()->where(['loai' => 'GOICAU_DANGLIENKET'])->all(), 'id_loai', 'ten'), ['prompt' => " -- Chọn --"]
            )
            ?>
        </div>
        <div class="col-lg-3">
            <?=
            $form->field($model, 'vatlieu')->dropDownList(
                    ArrayHelper::map(CauThongtinchung::find()->where(['loai' => 'GOICAU_VATLIEULIENKET'])->all(), 'id_loai', 'ten'), ['prompt' => " -- Chọn --"]
            )
            ?>
        </div>
        <div class="col-lg-3">
